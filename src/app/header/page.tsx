...
const Header = ({ getInTouchClick }: any) => {
  ...
  return (
    <div
      id="header"
      className={`fixed top-2 w-full lg:w-[calc(100%-4rem)] z-50 rounded-2xl px-4 py-2 ${
        scrolled ? "bg-slate-800 " : "bg-slate-400/1"
      }`}
    >
        ...
        <div className="flex items-center mr-10">
          <Link href="/">
            ...
            <div className="flex items-center">
              <div className={`flex flex-col items-center p-1 border-dashed ${ scrolled ? "bg-green-600" : "bg-transparent"}`}>
                <h1 className="text-white text-2xl font-bold font-serif">
                  LR technosphere
                </h1>
                <p className={` text-xs italic ${ scrolled ? " text-slate-800" : "text-slate-800"}`}>
                  Defining Tomorrow
                </p>
              </div>
            </div>
        ...
        <nav className=" flex items-center text-white">
          <div className="hidden lg:flex gap-8 items-center">
            {navItems.map((item, index) => (
              <div
                key={index}
                className={`transition-all duration-500 text-base ${
                  scrolled ? " py-4" : " py-8"
                }`}
              ...
              >
                ...
                {item.dropdownContent && hoveredNavItem === index && (
                  <div
                    ...
                    style={{
                      transform: `translateY(${scrolled ? "65px" : "90px"})`,
                    }}
                    onMouseEnter={() => setHoveredNavItem(index)}
                    onMouseLeave={() => setHoveredNavItem(null)}
                  >
                    <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                      {item.dropdownContent.map((service, i) => (
                        <Link
                          ...
                          className="contents"
                        >
                          <li className="flex items-stretch bg-gray-100 rounded hover:bg-gray-200">
            ...
            <button
              className=" bg-green-600 border-white border-4 text-white text-lg px-6 py-1 rounded-2xl hover:bg-green-700 mt-3 animate-bounce whitespace-nowrap"
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden">
          <div className="fixed top-0 left-0 w-full h-fit pb-8 bg-slate-800 z-50 flex flex-col items-center pt-12 overflow-y-auto">
